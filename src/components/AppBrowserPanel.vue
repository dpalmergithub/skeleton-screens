<template>
    <div class="app-panel shadow">
        <div class="app-panel-header d-flex align-items-center">
            ...
            <b-button class="ml-auto mr-1 btn-sm" variant="primary" @click="triggerModal()">Source</b-button>
        </div>
        <div class="pb-5">
...
<script>
    export default {
        name: 'AppBrowserPanel',
        props: ["sourceCode"],
        methods: {
            triggerModal() {
                this.$root.$emit("changeSourceCode", this.sourceCode)
            }
        }
    }
</script>
<style scoped>
    .app-panel {
        border: thin solid rgba(0, 0, 0, .10);
        border-radius: 6px;
        width: 100%;
        min-height: 250px;
        margin: 0 auto;
